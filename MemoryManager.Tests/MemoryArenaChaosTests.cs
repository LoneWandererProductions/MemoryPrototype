...
namespace MemoryManager.Tests
{
    ...
    public class MemoryArenaChaosTests
    {
        ...
        public unsafe void Arena_HandleRecycling_IncrementsVersion_BlocksStaleHandles()
        {
            ...
            var oldPtr = (int*)fastLane.Resolve(oldHandle);
            *oldPtr = 42;
            ...
            Assert.AreEqual(oldHandle.Id, newHandle.Id,
                "The allocator should recycle tracking IDs to prevent footprint bloat.");
            Assert.AreNotEqual(oldHandle.Version, newHandle.Version,
                "Recycled IDs must feature an incremented generation value.");

            var newPtr = (int*)fastLane.Resolve(newHandle);
            *newPtr = 99; // Write new distinct data to the recycled slot

            // Act & Assert

            // Verifying the new handle operates perfectly
            var verifyPtr = (int*)fastLane.Resolve(newHandle);
        ...
        public void Arena_AbsoluteSaturation_HandlesCompactionAndThrowsOOM()
        {
            ...
            var physicalBlockSize = MemoryCanary.GetPhysicalSize(elementSize);

            // Establish a strict budget that fits exactly 5 aligned physical blocks
            var customCapacity = physicalBlockSize * 5;
            ...
            Assert.AreEqual(0, fastLane.FreeSpace(),
                "Lane should have exactly 0 bytes remaining at absolute structural saturation.");
            ...
            Assert.IsTrue(fastLane.HasHandle(handles[0]),
                "Survivor ID 0 must maintain valid entry properties post-compaction.");
            Assert.IsTrue(fastLane.HasHandle(handles[2]),
                "Survivor ID 2 must maintain valid entry properties post-compaction.");
            Assert.IsTrue(fastLane.HasHandle(handles[4]),
                "Survivor ID 4 must maintain valid entry properties post-compaction.");
